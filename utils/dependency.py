...
def drawNecessary1(Layers, adj_list, n, m, category):
    ...
    selected_indices = np.random.choice(
        len(left_instance_para), num_to_add, replace=False
    )
...
def draw_necessary2(Gnece2_d):
    ...
    def next2(Topo, G_topo):
        ...
        return [
            a for a in G_topo if not any(a in Gnece3_d[b] for b in G_topo if b != a)
        ]
    ...
    while True:
        ...
        if not Topo:
            ...
            possible = list(
                set(next1(Topo, G_topo)).intersection(set(next2(Topo, G_topo)))
            )  # intersection: rid some of intermediate abstract para
...
def draw_unnecessary(adj_list, Layers, G_d_nece, category):
    ...
    while any(param not in G_d_nece_copy for param in all_instance_para):
        ...
        a = random.choice(
            [param for param in all_instance_para if param not in G_d_nece_copy]
        )
...
def DrawAll(
    ...
    while retry_count < max_retries:
        ...
        question, solution, num_operation = question_solution(
            G_d, Gnece_d, Topo, category
        )
        ...
        return {
            "question": structure_des + question if include_background else question,
            "solution": solution,
            "num_operation": num_operation,
            # "graph": G_d,
            # "necessary_graph": Gnece_d,
            ...
            "w1": int(w1),
            "e": int(e),
            # "metadata": {"s": s, "n": n, "m": m, "d": d, "w0": w0, "w1": w1, "e": e},
        }

    # If all retries fail, raise an exception
    raise RuntimeError("Failed to generate valid data after maximum retries.")
